...
export async function POST(req: NextRequest) {
  try {
    ...
    const { guideline, document, mockMode, documentPdfText, fileName, extension, pageCount } = body as {
      guideline?: string;
      document?: string;
      mockMode?: boolean;
      documentPdfText?: string;
      fileName?: string;
      extension?: string;
    ...
    const hasPdfText = Boolean(documentPdfText);
    const textToInspect = (hasPdfText ? documentPdfText : document) ?? '';
    ...
    const prompt = buildPrompt(guideline, textToInspect, hasPdfText, safeFileName, safeExtension, pageCount ?? null);
